...
function p3_drawTile(i, j) {
  ...
  if(noiseVal<.5){
    ...
    terrainType = 'sand';
    colorVal = color(map(noiseVal,.5,1,235,3),map(noiseVal,.5,1,231,158),0);
    fill(colorVal);
  }
  else{
    terrainType = 'grass'
    colorVal = color(map(noiseVal,0,1,3,15),map(noiseVal,0,1,150,255),map(noiseVal,0,1,0,23));
    fill(colorVal);
  }
  ...
  if(terrainType=='sand'){
    yOffset += map(noiseVal,0,1,0,10)
    extrude_tile(yOffset,colorVal);
  }
  if(terrainType=='grass'){
    yOffset += map(noiseVal,.6,.8,5,25)
    extrude_tile(yOffset,colorVal);
  }
...
function drawTree(){
  ...
  let leavesColor = color('green');
  let ls = 5
  fill(modifyColor(leavesColor,.2,0));
  beginShape();
  angle = radians(360/6) + radians(30);
  x = cos(angle) * (tw/ls);
  y = sin(angle) * (th/ls);
  ...
  fill(modifyColor(leavesColor,.2,1));
  beginShape();
  angle = radians(360) + radians(30);
  x = cos(angle) * (tw/ls);
  y = sin(angle) * (th/ls);
