...
function p3_drawTile(i, j) {
  ...
  if(XXH.h32("tile:" + [i, j], worldSeed) % 6!=0){
    ...
    terrainType = 'snow'
    colorVal = color(map(noiseVal,0,1,200,255),map(noiseVal,0,1,200,255),map(noiseVal,0,1,200,255));
    fill(colorVal);
  }
  let n = clicks[[i, j]] | 0;
  if(n%2==1){
  ...
  if(terrainType=='snow'){
    yOffset += map(noiseVal,0,0.5,0,1) + (sin((millis()/500)+XXH.h32("tile:" + [i, j], worldSeed) % 4)*2)+10 
